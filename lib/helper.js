...
class Helper {
    ...
    async createNode(nodeIdOriginal, element) {
        try {
            ...
            if (valuesOnly != null && typeof valuesOnly === "object" && valuesOnly.length > 0) {
                // Meter-Channel einmal pro Node anlegen, falls Meter-Werte vorhanden sind
                const hasMeterValues = valuesOnly.some(v => (v.commandClassName ?? '').toLowerCase() === 'meter');
                if (hasMeterValues) {
                    const meterChannelPath = utils.deleteLastDot(utils.formatObject(`${nodeId}.Meter`));
                    if (!this.alreadyCreatedObjects[meterChannelPath]) {
                        this.alreadyCreatedObjects[meterChannelPath] = {};
                        await this.adapter.setObjectNotExistsAsync(meterChannelPath, {
                            type: 'channel',
                            common: {
                                name: 'Meter',
                            },
                ...
                for (const v of valuesOnly) {
                    ...
                    if (constant.noInfoDP.includes(commandClassName) || constant.noInfoDP.includes(v.propertyName)) {
                        continue;
                    }

                    // Meter: propertyKeyName_commandClass_endpoint_propertyKey
                    if (commandClassName.toLowerCase() === 'meter') {
                        if (v.propertyKeyName && typeof v.propertyKeyName === 'string' && v.propertyKeyName.includes('UNKNOWN')) {
                            this.adapter.log.warn(`Node ${nodeIdOriginal}: Unknown propertyKeyName "${v.propertyKeyName}" for ${v.commandClassName}.${v.propertyName}`);
                            continue;
                        }

                        const meterParts = [];
                        if (v.propertyKeyName != null && v.propertyKeyName !== '') {
                            meterParts.push(v.propertyKeyName);
                        } else if (v.propertyName != null && v.propertyName !== '') {
                            meterParts.push(v.propertyName);
                        }
                        meterParts.push(v.commandClass);
                        if (v.endpoint != null && v.endpoint > 0) {
                            meterParts.push(v.endpoint);
                        }
                        if (v.propertyKey != null && v.propertyKey !== '') {
                            meterParts.push(v.propertyKey);
                        }

                        let parsePath = `${nodeId}.Meter.${meterParts.join('_')}`;
                        parsePath = utils.deleteLastDot(utils.formatObject(parsePath));

                        // ab hier DP (Meter-spezifisch)
                    ...
                    const meaningfulKeys = constant.MEANINGFUL_PROPERTY_KEYS[v.commandClassName];
                    if (v?.propertyKeyName && meaningfulKeys && (meaningfulKeys.includes('*') || meaningfulKeys.includes(v.propertyName))) {
                        parsePath = `${parsePath}.${v.propertyKeyName.toLowerCase()
                            .replace(/[^\p{L}\p{N}\s]/gu, "")
                            .replace(/\s+/g, " ")
                            .trim()}`;
                    }
    ...
    async createReadyStatus(nodeId) {
        // extendObjectAsync statt setObjectNotExistsAsync verwenden, damit bestehende
        // Objekte aus älteren Adapter-Versionen (ohne common.type) korrigiert werden.
        await this.adapter.extendObjectAsync(`${nodeId}.ready`, {
            type: 'state',
            common: {
                id: 'ready',
                name: 'ready',
        ...
        await this.adapter.extendObjectAsync(`${nodeId}.status`, {
            type: 'state',
            common: {
                id: 'status',
                name: 'status',
        ...
        await this.adapter.extendObjectAsync(`${nodeId}.reInterview`, {
            type: 'state',
            common: {
                id: 'reInterview',
                name: 'Re-Interview',
