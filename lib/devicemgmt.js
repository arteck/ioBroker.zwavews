...
class dmZwave extends dmUtils.DeviceManagement {
            ...
            const res = {
                ...
                actions: [
                    ...
                    },
                    {
                        id: 'reinterview',
                        icon: 'refresh',
                        description: 'Re-interview',
                        handler: async (_id, context) => this.reInterviewNode(context, nodeId),
                    },
                ],
            };
    ...
    /**
     * Triggers a re-interview of a Z-Wave node via the WebSocket connection.
     *
     * @param {object} context - The device management context used to show messages.
     * @param {string} nodeId - The node ID to re-interview.
     */
    async reInterviewNode(context, nodeId) {
        try {
            const sendMessageAllowed = await this.adapter.getStateAsync('info.sendMessageAllowed');
            if (!sendMessageAllowed || sendMessageAllowed.val !== true) {
                await context.showMessage(
                    this.adapter.i18nTranslation?.['Send message is not allowed'] || 'Send message is not allowed',
                );
            ...
            if (!this.adapter.websocketController) {
                await context.showMessage(
                    this.adapter.i18nTranslation?.['WebSocket not available'] || 'WebSocket not available',
                );
                return { refresh: false };
            }
            ...
            this.adapter.websocketController.send(JSON.stringify(message));
            this.adapter.log.info(`reInterviewNode: Re-interview command sent for node ${nodeId}`);
            await context.showMessage(
                this.adapter.i18nTranslation?.['Re-interview started'] || `Re-interview started for node ${nodeId}`,
            );
        } catch (e) {
            this.adapter.log.error(`reInterviewNode error: ${e.message}`);
            await context.showMessage(
                this.adapter.i18nTranslation?.['Re-interview failed'] || `Re-interview failed: ${e.message}`,
            );
        }
        return { refresh: false };
    }
