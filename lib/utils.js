...
/**
 * Cleans a single object-name segment: removes non letter/number/space
 * characters, collapses whitespace and trims.
 *
 * @param {*} str - The raw segment.
...
/**
 * Builds the ioBroker object path for a Z-Wave value/notification event.
 *
 * This is the single source of truth for the path logic used by both the
 * production adapter (main.js) and the unit-test runner. It reproduces the
 * exact behaviour previously inlined in main.js.
 *
 * @param {string} nodeId - The formatted node ID (e.g. "nodeID_005").
 * @param {object} nodeArg - The event args (commandClassName, propertyName, ...).
 * @param {object} constant - The constants module (for MEANINGFUL_PROPERTY_KEYS).
 * @returns {{path: string, skip: boolean, meter: boolean, updateDevice: boolean}}
 *   path         - the resolved object path (empty string when skip=true),
 *   skip         - true when the event must be ignored (e.g. UNKNOWN meter key),
 *   meter        - true when the value belongs to the Meter command class,
 *   updateDevice - true when the caller must also update the device name (CC 119 name).
 */
function buildValueEventPath(nodeId, nodeArg, constant) {
    const result = { path: "", skip: false, meter: false, updateDevice: false };

    // ── Meter: propertyKeyName/propertyName_commandClass_endpoint_propertyKey ──
    if (nodeArg.commandClassName?.toLowerCase() === "meter") {
        result.meter = true;

        if (
            nodeArg.propertyKeyName &&
            typeof nodeArg.propertyKeyName === "string" &&
            nodeArg.propertyKeyName.includes("UNKNOWN")
        ) {
            result.skip = true;
            return result;
        }
        ...
        result.path = `${nodeId}.Meter.${meterParts.join("_")}`;
        return result;
    }

    // ── Standard command classes ──
    let path = `${nodeId}.${nodeArg.commandClassName}.${cleanSegment(nodeArg.propertyName)}`;

    const meaningfulKeys = constant.MEANINGFUL_PROPERTY_KEYS[nodeArg.commandClassName];
    if (
        nodeArg?.propertyKeyName &&
        meaningfulKeys &&
        (meaningfulKeys.includes("*") || meaningfulKeys.includes(nodeArg.propertyName ?? nodeArg.property))
    ) {
        path = `${path}.${cleanSegment(String(nodeArg.propertyKeyName).toLowerCase())}`;
    }

    path = deleteLastDot(formatObject(path));

    // CC 119 (Node Naming and Location)
    if (nodeArg.commandClass === 119) {
        switch (nodeArg.property) {
            case "name":
                result.updateDevice = true;
                path = `${nodeId}.info.${nodeArg.property}`;
                break;
            case "location":
                break;
            default:
    ...
    if (path.includes("firmwareVersions")) {
        path = `${path}_value`;
    }

    if (nodeArg.endpoint != null && nodeArg.endpoint > 0) {
        path = `${path}_${nodeArg.endpoint}`;
    }

    result.path = deleteLastDot(path);
    return result;
}

/**
 * Generates a reasonably unique message ID for zwave-js-server commands.
 *
